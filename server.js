...
db.serialize(() => {
  ...
  const stmt = db.prepare(`
    INSERT INTO terms_text (lang_code, content) 
    VALUES (?, ?)
  `);
  
  Object.entries(terms).forEach(([lang, text]) => {
    stmt.run(lang, text); 
  });
...
app.get('/api/terms', (req, res) => {
  ...
  db.get(
    'SELECT content FROM terms_text WHERE lang_code = ?',
    [lang],
    (err, row) => {
      if (err) return res.status(500).send(err.message);
      res.json({ content: row?.content || 'No terms found.' });
    }
  );
});
